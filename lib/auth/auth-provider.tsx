...
const signUpSchema = z.object({
  ...
  message: 'Academic year is required if you are not graduated',
  path: ['academicYear'],
});
...
interface AuthContextType {
  ...
  signUp: (payload: SignUpPayload) => Promise<{ success: boolean; error?: string; requiresConfirmation?: boolean; }>;
  ...
  const signUp = async (payload: SignUpPayload) => {
    try {
      ...
      const { data: authData, error } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            full_name: normalizedPayload.fullName.trim(),
            phone: normalizedPayload.phone.trim(),
            academic_year: normalizedPayload.isGraduated ? null : normalizedPayload.academicYear,
      ...
      if (error) {
        console.log(`[AUTH] Sign up failed: ${normalizedPayload.email} - ${error.message}`);
      ...
      if (authData.user && authData.user.identities && authData.user.identities.length === 0) {
        console.log(`[AUTH] Existing user signup attempt: ${normalizedPayload.email}`);
        return { 
          success: false, 
          error: 'An account with this email already exists. Please sign in instead.' 
        };
      ...
      console.log(`[AUTH] Sign up success: ${normalizedPayload.email} - needs confirmation: ${!authData.session}`);
      ...
      // If sign-up created an immediate session, persist profile fields in users table now.
      if (authData.session) {
        try {
          await fetch('/api/user', {
            ...
            body: JSON.stringify({
              ...
              academicYear: normalizedPayload.isGraduated ? null : normalizedPayload.academicYear,
              isGraduated: normalizedPayload.isGraduated,
              specialization: normalizedPayload.specialization,
              email: normalizedPayload.email,
            }),
          });
        } catch (syncError) {
          console.error('[AUTH] Post-signup profile sync failed:', syncError);
        }
      }
